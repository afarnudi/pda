...
ODACH_CC.initialize <- function(ipdata, control, config) {
  ...
  matches <- stringr::str_match_all(control$outcome, "\\(([^\\)]+)\\)")[[1]][, 2]
  surv_obj_elements <- stringr::str_trim(unlist(stringr::str_split(matches, ",")))
  surv_obj_type <- length(surv_obj_elements)
  ...
  if (surv_obj_type == 2) {
    ...
    formula <- as.formula(paste(response, paste(risk_factors, collapse = "+")))
    fit_i <- survival::cch(
      formula,
      data = ipdata, subcoh = ~subcohort,
      id = ~ID,
      cohort.size = full_cohort_size,
    ...
    init <- list(
      bhat_i = fit_i$coef,
      Vhat_i = summary(fit_i)$coef[, "robust se"]^2, # not as glm, coxph summary can keep NA's! but vcov fills 0's!
      site = config$site_id,
      site_size = nrow(ipdata),
      full_cohort_size = full_cohort_size,
      method = control$method
