...
fit.pool <- cch_pooled(Surv(time, status) ~ X1+Group+Category, data=odach_cc,  
...
# cch each site
sapply(1:3, function(i) cch(Surv(time, status) ~ X1 +Group+Category, 
...
control <- list(project_name = 'ODACH case-cohort toy example',
                ...
                variables_lev = list(Group=c('A','B','C'), Category=c('X','Y','Z')),  
